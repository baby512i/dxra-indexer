...
export function detectCLMM(tx: any): Pool | null {
  try {
    ...
    const hasCreatePool = logMessages.some(
      (log) => log.includes('create_pool') || log.includes('initialize_pair'),
    );
    ...
    let programId = RAYDIUM_MAINNET_PROGRAMS.CLMM;

    const accountKeys = tx.transaction?.message?.accountKeys || [];
    const isDevnet = accountKeys.some(
      (key: any) => key === RAYDIUM_DEVNET_PROGRAMS.CLMM,
    );

    if (isDevnet) {
      network = 'devnet';
      programId = RAYDIUM_DEVNET_PROGRAMS.CLMM;
    }
    ...
    console.error('Error detecting CLMM pool:', error);
    return null;
  }
}
